...
def letterbox(original_img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    # Resize and pad image while meeting stride-multiple constraints
    shape = original_img.shape[:2]  # current shape [height, width]
    ...
    if shape[::-1] != new_unpad:  # resize
        original_img = cv2.resize(original_img, new_unpad, interpolation=cv2.INTER_LINEAR)
    top, bottom = int(round(dh - 0.1)), int(round(dh + 0.1))
    left, right = int(round(dw - 0.1)), int(round(dw + 0.1))
    img = cv2.copyMakeBorder(original_img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)  # add border
